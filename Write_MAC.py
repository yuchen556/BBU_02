    ...
    def test_content(self):
        ...
        while (i < 5):
            stdin, stdout, stderr = ssh.exec_command(
                "projects/bbu_server01/LAN_BBU/eeupdate64e /NIC=%d /MAC=%s " % (i + 1, self.mac_address[i]))
        ...
        while (j < 5):
            stdin2, stdout, stderr2 = ssh.exec_command(
                "projects/bbu_server01/LAN_BBU/eeupdate64e /NIC=%d /MAC_DUMP | grep MAC" % (j + 1))
            ...
            with open(self.logname, 'a+') as f:
                f.write("Get MAC number %d is: %s" % (j + 1, MAC_number[j]))
